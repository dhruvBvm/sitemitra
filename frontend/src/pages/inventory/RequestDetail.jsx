    ...
    <div className="flex flex-col min-h-screen space-y-4 max-w-3xl mx-auto p-4 overflow-y-auto pb-20">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-[#1F2937]">Request Details</h1>
            <p className="text-sm text-[#6B7280]">{request.requestNo}</p>
          </div>
        </div>
      </div>

      {showApproveReject && (
        <div className="fixed bottom-[64px] left-0 right-0 bg-white border-t border-[#E5E7EB] p-3 flex justify-between gap-3 z-40 max-w-[428px] mx-auto shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
          <Button className="flex-1 bg-[#2563EB] hover:bg-[#2563EB] text-white py-2.5 px-4 rounded-[16px]" onClick={() => openModal('approve')}>
            Approve
          </Button>
          <Button className="flex-1 text-[#EF4444] border-[#EF4444] hover:bg-red-50 py-2.5 px-4 rounded-[16px]" variant="outline" onClick={() => openModal('reject')}>
            Reject
          </Button>
        </div>
      )}
      ...
      <div className="bg-white rounded-[20px] shadow-sm border border-transparent p-5">
        ...
        {request.approvalHistory && request.approvalHistory.length > 0 ? (
            ...
            {request.approvalHistory.map((h, i) => (
              <div key={i} className="flex gap-4 items-start">
                ...
                <div className="flex-1 pb-4">
                  <div className="bg-[#f8faff] border border-transparent rounded-[16px] p-3 w-full">
          ...
          <p className="text-sm text-[#6B7280] italic bg-[#f8faff] p-4 rounded-[16px] border border-transparent text-center">Pending approvals</p>
        )}
      </div>
