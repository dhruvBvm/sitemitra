...
export default function MaterialHistory() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white w-full pb-24">
      
      {/* STICKY TOP SECTION */}
      <div className="sticky top-14 z-30 bg-white pb-3 -mx-4 px-4 border-b border-slate-100 shadow-sm">
        {/* Top Navigation */}
        <div className="bg-white py-[12px] border-b border-[#E5E7EB] flex items-center mb-2">
          <button onClick={() => navigate(-1)} className="p-1 mr-2 rounded-full hover:bg-[#F3F4F6] transition-colors text-[#6B7280]">
            <ArrowLeft className="w-6 h-6" />
          </button>
          <h1 className="text-[20px] font-bold tracking-tight text-[#1F2937] capitalize">Material History</h1>
        </div>

        {/* Header Area */}
        <div className="mb-3 bg-white rounded-[20px] shadow-sm border border-transparent p-3 text-center">
          <h2 className="text-lg font-bold text-[#1F2937] mb-1">{data.materialName}</h2>
          <p className="text-xs text-[#6B7280] mb-2">{data.costCode}</p>
          <div className="py-2 bg-[#f8faff] rounded-[16px] border border-transparent flex flex-col items-center justify-center">
        ...
        <div className="mb-3">
          <div className="flex gap-2 w-full">
            <div className="flex-1 bg-white rounded-[20px] shadow-sm border border-transparent p-2 flex flex-col justify-center items-center">
        ...
        <div className="flex gap-2 w-full">
          <button 
            className={`flex-1 py-[6px] px-3 text-[11px] sm:text-xs font-bold rounded-[20px] transition-colors whitespace-nowrap ${activeTab === 'ALL' ? 'bg-[#2563EB] text-white' : 'bg-[#F3F4F6] text-[#1F2937] hover:bg-[#F3F4F6]'}`}
            onClick={() => setActiveTab('ALL')}
          >
            ALL ENTRIES
          </button>
          <button 
            className={`flex-1 py-[6px] px-3 text-[11px] sm:text-xs font-bold rounded-[20px] transition-colors whitespace-nowrap ${activeTab === 'IN' ? 'bg-[#2563EB] text-white' : 'bg-[#F3F4F6] text-[#1F2937] hover:bg-[#F3F4F6]'}`}
            onClick={() => setActiveTab('IN')}
          >
            STOCK IN
          </button>
          <button 
            className={`flex-1 py-[6px] px-3 text-[11px] sm:text-xs font-bold rounded-[20px] transition-colors whitespace-nowrap ${activeTab === 'OUT' ? 'bg-[#2563EB] text-white' : 'bg-[#F3F4F6] text-[#1F2937] hover:bg-[#F3F4F6]'}`}
        ...
        <div className="space-y-3 pb-6">
          {filteredHistory.length > 0 ? (
            filteredHistory.map((item, index) => (
              <div key={index} className="bg-white rounded-[20px] shadow-sm border border-transparent p-[14px] flex flex-col active:scale-[0.99] transition-transform cursor-default">
            ...
            <div className="text-center py-10 bg-white rounded-[20px] border border-dashed border-[#E5E7EB]">
              ...
              <p className="text-[#6B7280] font-medium">No entries found.</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="fixed bottom-[64px] left-0 right-0 max-w-[428px] mx-auto bg-white border-t border-[#E5E7EB] p-2 z-40 shadow-sm flex gap-2">
        <button 
          className="flex-1 bg-blue-50 text-[#2563EB] border border-blue-200 hover:bg-blue-100 font-bold py-[8px] px-[12px] rounded-[16px] transition-colors flex justify-center items-center text-[12px] sm:text-[14px] whitespace-nowrap"
        ...
        <button 
          className="flex-1 bg-red-50 text-[#EF4444] border border-red-200 hover:bg-red-100 font-bold py-[8px] px-[12px] rounded-[16px] transition-colors flex justify-center items-center text-[12px] sm:text-[14px] whitespace-nowrap"
