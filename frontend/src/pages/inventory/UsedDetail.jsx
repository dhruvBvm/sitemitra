  ...
  if (loading) {
    return (
      <div className="sticky top-0 left-0 right-0 mx-auto max-w-[428px] z-40 bg-white border-b border-[#E5E7EB] overflow-x-hidden">
        <div className="max-w-[428px] mx-auto px-4 py-2 flex items-center gap-2">
      ...
      <>
        <div className="sticky top-0 left-0 right-0 mx-auto max-w-[428px] z-40 bg-white border-b border-[#E5E7EB] overflow-x-hidden">
          <div className="max-w-[428px] mx-auto px-4 py-2 flex items-center gap-2">
            <button
              onClick={() => navigate(-1)}
              className="p-1.5 bg-[#f8faff] text-[#6B7280] rounded-full hover:bg-[#F3F4F6] transition-colors shrink-0"
            >
              <ArrowLeft className="w-5 h-5" />
            </button>
            <div>
              <h1 className="text-[1.1rem] font-bold tracking-tight text-[#1F2937]">Used Details</h1>
  ...
  return (
    <>
      ...
      <div className="sticky top-0 left-0 right-0 mx-auto max-w-[428px] z-40 bg-white border-b border-[#E5E7EB] overflow-x-hidden">
        <div className="max-w-[428px] mx-auto px-4 py-2 flex flex-col gap-1.5 justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-[1.1rem] font-bold tracking-tight text-[#1F2937]">Consumption Details</h1>
      ...
      <div className="flex flex-col  space-y-4 max-w-[428px] mx-auto px-4 pb-24 pt-4">
          ...
          {entry.notes || entry.userNotes ? (
            <div className="space-y-2">
              {(entry.userNotes || entry.notes) && (
                <div>
                  <p className="text-sm text-[#1F2937] bg-[#f8faff] p-2 rounded-lg flex items-start gap-2 border border-transparent">
            ...
            <p className="text-sm text-[#6B7280] italic bg-[#f8faff] p-2 rounded-lg border border-slate-100 text-center">
              No notes provided
            </p>
          )}
        </div>
        ...
        {/* Materials Used */}
        <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-3">
          <h2 className="text-sm font-bold text-green-800 uppercase tracking-wider mb-4">Materials Used</h2>
            ...
            {entry.materials?.map((mat, i) => (
              <div key={i} className="flex flex-col p-3 rounded-lg border border-slate-100 bg-gray-100">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-sm font-bold text-white bg-green-700 px-2 py-0.5 rounded">
                    {mat.quantity || mat.qty}{' '}
                    <span className="text-xs text-green-100 font-medium">{mat.unit}</span>
                  </span>
                </div>
                <div className="mt-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {mat.imageUrls.map((img, idx) => (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="w-16 h-16 rounded border border-transparent overflow-hidden hover:scale-105 transition-transform block"
                        >
            ...
            <div className="flex flex-wrap gap-2">
              {entry.imageUrls.map((img, idx) => (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="w-20 h-20 rounded-md border border-transparent overflow-hidden hover:scale-105 transition-transform block"
                >
